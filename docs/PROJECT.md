...
21. **Scope realization: surface-level optimization, not systematic.** v1 used 5 ad-hoc hyperparameter tweaks rather than a real grid search; one calibration method (threshold gating on P(T)) rather than a comparison vs temperature scaling / isotonic / per-class; one architecture (ModernBERT) rather than cross-architecture validation; one eval split rather than full 5-fold CV with per-breakdown reporting. Proposed framework in STATE.md "Systematization roadmap" to be built before release #2+. Investment now compounds across releases #2-#10.

---

*End of plan. Next concrete step: run `scripts/prepare_data.py` (see [STATE.md](STATE.md)), eyeball the output distribution, then write `scripts/train_encoder.py` to fine-tune ModernBERT-base on the produced splits.*
