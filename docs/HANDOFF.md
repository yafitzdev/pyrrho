...
- **V7 generation in progress (local vault only, not yet on HF):** SDGP cell-targeted pass 1 added **745 fresh cases** on top of V6 (vault total 3,725). All existing cases tagged `meta.dataset_version: "v6"`; new ones tagged `"v7"`. 0 parse/checker/dup failures across the 745 ingested. Pass 1 stopped at monthly usage cap on Sonnet. Next pass after window reset → continue filling top-priority gap cells toward the 5K-10K MoE-pre-training target.
