...
1. ~~**Phase 0: V5.1 schema enrichment**~~ **COMPLETE (2026-05-20).** All 2,980 V5.1 cases LLM-enriched with V6+ schema fields (query_rewritten, context summaries, governance signals, boundary_proximity, near_miss_reason) via Sonnet subagents + LM Studio local worker. 0 TODO markers remain in vault. See LOG 2026-05-20 evening.
   ...
   **Phase 0c: V6 completion** (in flight 2026-05-20 evening) — adds the 4 MoE-training fields not yet in V6: per-chunk `boundary_quality`, per-case `evidence_bias_score`, per-case `input.evidence_chain.order[]` (multi-chunk only), and per-case `meta.grounding_targets` (gold_answer + per-sentence attributions, TRUSTWORTHY only). Tooling: `fitz_gov/sdgp/llm_enrich_v6.py` + `scripts/sdgp_enrich_v6_complete.py`. LM Studio worker (qwen3.6-35b@Q5) running on 1,483 t1 cases in background (background task `bltlcb9w5`), ETA ~6.5h at ~16s/case. Remaining 1,483 cases (incl. all 60 tier0) on the Sonnet side, not yet launched. After completion: re-run `scripts/sdgp_upload_v6_hf.py` to refresh `yafitzdev/fitz-gov` v6.0.0 in place.
