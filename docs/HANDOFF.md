...
- Initial V8 taxonomy-gap implementation is in fitz-gov: five new primary patterns (`resolved_candidate_selection`, `verdict_conflict`, `authority_status_conflict`, `version_build_mismatch`, `missing_execution_result`) expanded across 7 current domains x 3 difficulties = 105 cells. The active local V8 set is now **840 QA-clean rows**: original 525 rows + 105 hard `verdict_conflict` rows + 210 repaired balanced controls for `resolved_candidate_selection` and `version_build_mismatch`. Plan file: `C:/Users/yanfi/PycharmProjects/fitz-gov/docs/V8_TAXONOMY_EXPANSION_PLAN.md`.
- Expansion rows are generated, merged, structurally audited, repaired, and blind-label clean locally: vault is **11,340 rows** = 10,500 V6/V7 + **840 V8**. Full clean score is **840/840 validated / 0 triage**, **0 missing / 0 invalid / 0 error** at `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_v8_qa/clean_840_score/`. Current clean manifest is `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_v8_qa/blind_label_manifest_clean_840.jsonl`.
- A separate Claude-generated candidate handoff exists at `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_handoff_v8_candidate_20260525_claude_expand/`. It is **not active data** and **not pyrrho training data**. Generation was still moving during the 2026-05-25 evening inspection; the 18:56 intake snapshot saw **89** main `batch_*.jsonl` files, **2,646** raw lines, **2,643** parseable unique candidate IDs, **0** duplicate IDs, **717** assigned slots still missing, **4** strict-read-fail output files, and **15** parsed rows missing core classification/domain/difficulty fields. Fast structural dry-run result after fixing the merge checker performance was **1,915 accepted / 0 existing / 624 rejected**. Normalize and run the clean testcase addition cycle before any merge.
...
- **fitz-gov V8 local rows**: the active local vault now has **840 V8 rows** at `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_vault_v51_enriched/cases.jsonl` (**11,340 total rows** including V6/V7). The clean training manifest is `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_v8_qa/blind_label_manifest_clean_840.jsonl`: original 525 QA-clean V8 rows + 105 QA-clean hard `verdict_conflict` rows + 210 QA-clean repaired balanced controls. Full V8 clean score at `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_v8_qa/clean_840_score/` is **840/840 agreement**, **0 missing / 0 invalid / 0 error**, **0 triage**. The failed pre-fix control attempt remains only as a historical artifact at `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_v8_qa/balanced_controls_repaired_clean_20260525/` (**148/210 agreement**) and must not be used.
- **fitz-gov V8 candidate handoff**: Claude generated additional V8 candidate rows under `C:/Users/yanfi/PycharmProjects/fitz-gov/data/sdgp_handoff_v8_candidate_20260525_claude_expand/`. This handoff is unvalidated and unmerged. The 2026-05-25 18:56 intake snapshot observed **2,643** parseable unique candidate IDs, **717** assigned slots still missing, **4** strict-read-fail files, **15** incomplete parsed rows, and a fast dry-run result of **1,915 accepted / 0 existing / 624 rejected**. Do not point pyrrho at this handoff or merge it into the active vault until structural dry-run and offline blind-label QA both pass.
...
4. **V8 next step** — active V8 data is clean at **840 rows** and `pyrrho-nano-g2.2` is trained. Do **not** publish yet. The aviation probe did not find a grave new domain-specific gap (`g2.2` scored **8.67/10**). If continuing V8 with Claude's new candidate handoff, first normalize and run the full clean testcase addition cycle; do not merge or train from raw candidates. The unresolved model-quality decision remains whether to patch known resolved-candidate / wrong-release boundaries (`ecu_02`, `ecu_07`, `air_02`) or stop V8 here and move to another domain probe.
