...
- **fitz-gov dataset on HF**: https://huggingface.co/datasets/yafitzdev/fitz-gov (public, CC BY-NC 4.0, **V6.0.0**, three configs: tier1_core / tier0_sanity / validation, 16.4 MB). V6 adds LLM-enriched signals to all 2,980 cases in two phases. Phase 0b core governance signals: `query_rewritten`, per-context `summary`/`relevance_to_query`/`anchor_period`, `governance.{hallucination_pressure, retrieval_retry_value, query_evidence_alignment, answer_coverage, boundary_proximity.distance}`, `meta.near_miss_reason`. Phase 0c MoE multi-task training ground truth: per-chunk `boundary_quality`, `governance.evidence_bias_score`, multi-chunk `input.evidence_chain`, TRUSTWORTHY `meta.grounding_targets` (gold_answer + per-sentence chunk attributions). Convenience top-level `label` and `tier` fields. Last refreshed 2026-05-21 (100% coverage).
- **V7 generation done (local vault only, not yet on HF):** SDGP cell-targeted generation across two passes added **1,400 fresh cases** on top of V6 (vault total **4,380**). All existing cases tagged `meta.dataset_version: "v6"`; new ones tagged `"v7"` + `version: "fitz-gov-7.0"`. **Zero parse/structural-checker/dedup failures across all 1,400 ingests.** Coverage: top 280 highest-priority gap cells × 5 cases each. Quality stayed pristine end-to-end. Next: continue filling toward 5K–10K cases for MoE multi-task pre-training; publish V7 to HF once additional QA pass (blind labeling + nightly spot-check) is wired up.
