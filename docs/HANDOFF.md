...
4. **Structured/code evidence is an unsafe OOD surface.** The full 20k QA-clean candidate probe scored **52.69%** calibrated accuracy / **51.69%** false-TRUSTWORTHY. Code is worse (**49.35% / 71.38% FT**) than structured (**56.02% / 31.99% FT**). Balancing by taxonomy pattern and by modality-pattern leaves results near **52.2%** accuracy and **50-51%** FT, so the gap is not only dataset skew.
...
33. **Next modality step:** build a pyrrho data-prep path that can consume candidate structured/code manifests without merging them into the active fitz-gov vault, then run controlled retraining probes: (a) unstructured+structured, (b) unstructured+code, (c) unstructured+structured+code, and (d) modality-specialist heads or separate specialist encoders. Report per-modality gates; do not treat a single aggregate score as sufficient.
