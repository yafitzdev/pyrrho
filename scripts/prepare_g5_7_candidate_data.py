"""Prepare the local pyrrho-nano-g5.7 V12.1 failure-repair candidate dataset.

This keeps the flattened rows and split assignments from
`data/multitask_g5_6_v12_focused_medium_candidate`, then appends the local
1,200-row V12.1/g5.7 failure-repair candidate block. The candidate block is
...
DEFAULT_CANDIDATE_DIR = (
    ...
    / "_workspaces"
    / "handoff"
    / "v12_1_g5_7_failure_repair_20260616"
    / "subagent_outputs"
)
DEFAULT_OUTPUT_DIR = Path("data/multitask_g5_7_v12_1_failure_repair_candidate")
DEFAULT_CONFIG = Path("configs/encoder/modernbert_base_g5_7_v12_1_failure_repair_candidate.yaml")
SOURCE_KIND = "fitz_gov_v12_1_g5_7_failure_repair_candidate_structural_clean_no_blind_qa"
CANDIDATE_STATUS = "structural_validation_clean_duplicate_clean_failure_repair_no_blind_qa"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-data-dir",
        type=Path,
        default=Path("data/multitask_g5_6_v12_focused_medium_candidate"),
        help="Flattened g5.6 multitask data dir.",
    )
    parser.add_argument(
        "--fitz-gov-repo",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="V12.1/g5.7 failure-repair candidate JSONL output directory.",
    )
...
def add_row_strict(
    ...
    if case_id in seen_ids:
        raise ValueError(f"duplicate row id while preparing g5.7 data: {case_id!r}")
...
def main() -> int:
    ...
    for path in candidate_files(candidate_dir):
        ...
        for raw in read_jsonl(path):
            ...
            if get_path(case, ("meta", "dataset_version")) != "v12_1_candidate":
                raise ValueError(f"{case.get('id')}: expected meta.dataset_version='v12_1_candidate'")
            ...
            flat["answerability_shape_raw_v12_1"] = raw_shape
            flat["answerability_shape_collapsed_v12_1"] = collapsed_shape
            flat["v12_1_filled_retrieval_control_fields"] = filled_fields
            flat["v12_1_candidate_dir"] = str(candidate_dir)
            flat["v12_1_candidate_status"] = CANDIDATE_STATUS
            add_row_strict(
                rows_by_split,
    ...
    out_metadata: dict[str, Any] = {
        "source": {
            ...
            "base_fitz_gov_version": "v11.0.0_plus_v12_g5_6_candidate",
            "candidate_dir": str(candidate_dir),
            "candidate_status": CANDIDATE_STATUS,
            "candidate_plan": (
                "data/_workspaces/reports/"
                "v12_1_g5_7_failure_repair_20260616_plan.json"
            ),
            "candidate_validation_report": (
                "data/_workspaces/handoff/v12_1_g5_7_failure_repair_20260616/"
            ...
            "target_profile": "failure_repair",
            "blind_label_qa": "not_run",
        },
        **summary,
        "candidate_v12_1_rows_read": candidate_rows,
        "candidate_v12_1_rows_by_split": dict(candidate_rows_by_split),
        "candidate_v12_1_rows_by_file": candidate_rows_by_file,
        "v12_1_raw_answerability_shape_counts": dict(raw_shape_counts),
        "v12_1_collapsed_answerability_shape_counts": dict(collapsed_shape_counts),
        "v12_1_filled_retrieval_control_field_counts": dict(filled_field_counts),
    ...
    write_manifest(
        ...
        extra={
            "script": "prepare_g5_7_candidate_data.py",
            "base_data_dir": str(base_dir),
            "candidate_dir": str(candidate_dir),
    ...
    print(f"v12.1 rows   : {candidate_rows}")
    print(f"v12.1 split  : {dict(candidate_rows_by_split)}")
    print(
